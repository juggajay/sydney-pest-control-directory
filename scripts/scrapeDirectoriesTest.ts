...
async function scrapeYellowPages(suburb: string): Promise<ScrapedBusiness[]> {
  ...
  try {
    ...
    console.log(`   YP content length: ${content.length} chars`);

    // Debug: Show first 3000 chars and lines with "pest"
    console.log('\n--- YP SAMPLE (first 3000 chars) ---');
    console.log(content.substring(0, 3000));
    console.log('--- END SAMPLE ---\n');

    // Debug: Find lines with business names
    const testLines = content.split('\n').filter(l => l.includes('[###') || (l.toLowerCase().includes('pest') && l.length < 100));
    console.log('--- Lines with [### or pest ---');
    testLines.slice(0, 20).forEach(l => console.log(`  LINE: ${l.substring(0, 120)}`));
    console.log('--- END ---\n');
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (nameCandidate) {
        ...
        if (currentBusiness.name && (currentBusiness.phone || currentBusiness.website)) {
          ...
          console.log(`   ✓ Saved: ${currentBusiness.name} (${currentBusiness.phone || currentBusiness.website})`);
        }

        // Check if this looks like a pest control business or is on a pest control listing page
        // Since we're on Yellow Pages pest control search, all results are pest control companies
        const isPestRelated = potentialName.toLowerCase().includes('pest') ||
                              potentialName.toLowerCase().includes('termite') ||
                              potentialName.toLowerCase().includes('exterminator') ||
                              potentialName.toLowerCase().includes('rentokil') ||
                              potentialName.toLowerCase().includes('flick');

        // Start new business - be more permissive since we're on a pest control search page
        if (isValidBusinessName(potentialName) && potentialName.length > 3) {
          currentBusiness = { name: potentialName, suburb };
          console.log(`   → New candidate: ${potentialName}`);
        } else {
          currentBusiness = {};
        }
      }
      ...
      // Skip [Call](tel:...) buttons - we want the actual number display
...
async function main() {
  console.log('🧪 TEST SCRAPE - Yellow Pages\n');
  console.log(`Testing ${TEST_SUBURBS.length} suburbs: ${TEST_SUBURBS.join(', ')}\n`);
  ...
  for (const suburb of TEST_SUBURBS) {
    ...
    const businesses = await scrapeYellowPages(suburb);
    await sleep(3000);
    ...
    for (const b of unique) {
      const exists = await checkExists(b.phone, b.website, b.name);
      console.log(`   ${exists ? '⏭️ EXISTS' : '✅ NEW'}: ${b.name} | ${b.phone || 'no phone'} | ${b.website || 'no website'}`);
    }
  }

  console.log('\n' + '='.repeat(50));
  console.log(`Total unique businesses found: ${totalFound}`);
  ...
  const { count } = await supabase
    ...
    .eq('status', 'active');
  console.log(`Current operators in DB: ${count}`);
}
